...
uint32_t* get_rgb_array(uint32_t r, uint32_t g, uint32_t b);
...
void hal_entry(void)
{
   ...
   R_BSP_NonSecureEnter();
    ...
    while (1)
    {
        ...
        uint32_t* grb_arr = get_rgb_array (p_ob1203_data.red_data, p_ob1203_data.green_data, p_ob1203_data.blue_data);
        ...
        for (uint32_t i = 0;i<3; i++)
        {
            ...
            sprintf (write_buffer, "rgb element: %lu \n\r", grb_arr[i]);
            console_write (write_buffer);
            /* Write to this pin */
            R_BSP_PinWrite ((bsp_io_port_pin_t) leds.p_leds[grb_arr[i]], BSP_IO_LEVEL_LOW);  // Turn on the current LED
...
// Function to get an array of RGB threshold results
uint32_t* get_rgb_array(uint32_t r, uint32_t g, uint32_t b)
{
    static uint32_t result[3]; // Fixed-size array to store results
    uint32_t r_threshold = 175;
    uint32_t b_threshold = 150;
    uint32_t g_threshold = 300;
    int idx = 0; // Keeps track of the number of elements added to the array
    ...
    // Fill the remaining slots with 0 or some invalid value to indicate unused slots

    result[idx++] = 0;

    return result; // Return the array (or pointer to the first element)
}
